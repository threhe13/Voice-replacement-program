...
def prepare_dirs(config, hparams):
    if hasattr(config, "data_paths"):
        ...
        dataset_desc = config.project_name
    ...
    if config.load_path:
        ...
        print('path:', os.path.join(config.model_dir, "hparams.py"))
        copy_file("tacotron/hparams.py", os.path.join(config.model_dir, "hparams.py"))

    model = Model.objects.get(name=config.project_name, dataset=config.datasets_name)
