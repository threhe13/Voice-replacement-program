...
from tacotron.utils.audio import save_wav, inv_spectrogram
from tacotron.text import sequence_to_text, text_to_sequence
from tacotron.datasets.datafeeder_tacotron2 import DataFeederTacotron2
import warnings
...
def train(log_dir, config):
    ...
    checkpoint_path = os.path.join(config.model_dir, 'model.ckpt')  # 'logdir-tacotron\\moon_2018-08-28_13-06-42\\model.ckpt'
    ...
    log(' [*] Using model: %s' % config.initialize_path)  # 'logdir-tacotron\\moon_2018-08-28_13-06-42'
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if step % config.checkpoint_interval == 0:
                    ...
                    saver.save(sess, config.model_dir, global_step=step)

                    form = Model.objects.get(uploader=config.username, dataset=config.datasets_name, name=config.project_name)
                    form.process = int((step/hparams.num_steps)*100)
                    form.save()
