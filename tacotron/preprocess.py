...
from voice.models import Audio
import warnings
warnings.simplefilter(action='ignore', category=FutureWarning)

def audio_path(username, project_name):
    return os.path.join(settings.MEDIA_ROOT, 'user_audio', username, project_name)

def preprocess(username, project_name):
    mod = importlib.import_module('tacotron.datasets.bisang')
    print(username, project_name)

    in_dir = audio_path(username, project_name)
    out_dir = os.path.join(settings.MEDIA_ROOT, 'user_prep', username, project_name)

    os.makedirs(out_dir, exist_ok=True)

    process = Audio.objects.get(uploader=username, name=project_name)
    process.processing = 1
    process.save()

    metadata = mod.build_from_path(hparams, in_dir, out_dir, project_name, num_workers=2, tqdm=tqdm)
...
if __name__ == "__main__":
    ...
    assert name in ["cmu_arctic", "ljspeech", "son", "moon", "kss", "bisang", 'test']
